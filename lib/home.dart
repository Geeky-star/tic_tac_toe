...
class _HomePageState extends State<HomePage> {
  ...
  void playGame(GameButton gb) {
    setState(() {
      if (activePlayer == 1) {
        gb.text = "X";
        gb.bg = Colors.purple[200];
        activePlayer = 2;
        player1.add(gb.id);
      } else {
        gb.text = "0";
        gb.bg = Colors.pink[200];
        activePlayer = 1;
        player2.add(gb.id);
      }
      gb.enabled = false;
      ...
      if (winner == -1) {
        if (buttonsList.every((p) => p.text != "")) {
          showDialog(
              context: context,
              builder: (_) =>
                  new CustomDialog("Game Tie", "Play again", resetGame));
        } else {
    ...
    if (winner != -1) {
      if (winner == 1) {
        showDialog(
            context: context,
            builder: (_) =>
                new CustomDialog("Player 1 Won", "Reset Game", resetGame));
      } else {
        showDialog(
            context: context,
            builder: (_) =>
                new CustomDialog("Player 2 Won", "Reset Game", resetGame));
      }
    }
